/**
 * Sell Payment Report — every customer payment instalment, with filters.
 *
 * Each row corresponds to one SalePayment received against a Sale. The
 * 'Action' column links to the parent sale detail page so you can drill in.
 *
 * Customer Group column is kept for layout parity with the inspiration
 * screenshot but rendered as '—' because IFFAA doesn't (yet) have a
 * CustomerGroup model. The filter dropdown is hidden until that ships.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import DateRangeField from '../../components/ui/DateRangeField'
import { Link } from 'react-router-dom'
import { getSellPaymentReport } from '../../api/reports'
...
export default function SellPaymentReportPage() {
  ...
  const fetchReport = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await getSellPaymentReport(params)
      setData(res)
    } catch (err) {
  ...
  }, [page, limit, customerId, locationId, method, dateFrom, dateTo, search])
  ...
  const onReset = () => {
    setCustomerId(''); setLocationId(''); setMethod(''); setSearch('')
  ...
  const customerOptions  = data?.customer_options ?? []
  ...
  const handlePrint = async () => {
    ...
    const custName = customerId ? (customerOptions.find((c) => c.id === customerId)?.name || '') : 'All Customers'
    ...
    </tr>`).join('') || '<tr><td colspan="8" class="empty">No payments for these filters.</td></tr>'

    const w = window.open('', '_blank', 'width=1250,height=900')
    if (!w) { window.alert('Allow popups to print this report.'); return }
    w.document.write(`<!doctype html><html><head><meta charset="utf-8">
<title>Sell Payment Report — ${esc(company?.business_name || '')}</title>
<style>
...
<div class="hdr">
  <div>
    <h1 class="title">Sell Payment Report</h1>
    <div class="meta">
...
<div class="filters">
  <div><div class="k">Customer</div>${esc(custName)}</div>
...
<div class="kpis">
  <div class="kpi"><div class="l">Total Collected</div><div class="v" style="color:#059669">${fmtBDT(footer.total_amount)}</div></div>
...
<table>
  <thead><tr>
    <th>#</th><th>Reference</th><th>Paid On</th><th>Customer</th><th>Group</th>
    <th>Method</th><th>Invoice</th><th class="num">Amount</th>
  </tr></thead>
  <tbody>${body}</tbody>
  ${rows.length ? `<tfoot><tr>
    <td colspan="7">TOTALS (all filtered rows · ${fmtInt(footer.count)} payments)</td>
    <td class="num">${fmtBDT(footer.total_amount)}</td>
  </tr></tfoot>` : ''}
</table>

<div class="footer">
  <div>Every payment received against sales in the selected period.</div>
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-600 to-cyan-500 px-6 py-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">Sell Payment Report</h1>
            <p className="text-xs text-emerald-50 mt-0.5">
              Every payment instalment received from customers against finalised
              sales, with filter by customer, location, method and date.
            </p>
          </div>
      ...
      {/* ── KPI strip — per-method roll-up ────────────────────────────────── */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Kpi label="Total received"
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <a
              href={csvHref}
              download={`sell-payments-${dateFrom}_${dateTo}.csv`}
      ...
      <tbody className="divide-y divide-gray-50">
        {rows.map((r) => (
          <tr key={r.id} className="hover:bg-emerald-50/40 transition-colors">
            ...
            <td className="px-5 py-3 text-gray-400">{r.customer_group || '—'}</td>
            <td className="px-5 py-3">
            ...
            <td className="px-5 py-3 font-mono text-xs text-gray-700">{r.invoice_number}</td>
          </tr>
        ))}
      </tbody>
      <tfoot>
        <tr className="border-t-2 border-emerald-200 bg-emerald-50/60 text-sm font-semibold text-emerald-900">
          ...
          <td className="px-5 py-3 text-xs uppercase tracking-wider" colSpan={4}>
...
function buildCsv(rows, footer = {}) {
  ...
  const header = ['Reference', 'Paid on', 'Amount', 'Customer', 'Method', 'Invoice', 'Location']
  const lines = rows.map((r) => [
    r.reference_no, r.paid_on, r.amount, r.customer_name,
    r.method_label, r.invoice_number, r.location_name,
  ].map(esc).join(','))
